import Submission.Abel.AreaPairing
import Submission.PlanarStokes.CompactSupport
...
* **`pairing_dbar_eq_zero`** (Forster 19.6's Stokes half, PoU-globalized): `∂̄`-exact
  ...
  per PoU chart, `ψᵢ ∂̄u θᵢ = ∂̄(ψᵢ u θᵢ) - ∂̄ψᵢ · u θᵢ`, the first integral dies by
  compact-support Stokes, and the leftover `∑ᵢ ∫ ∂̄ψᵢ · u θ` dies after inserting `∑ⱼ ψⱼ = 1`,
  ...
  `∑ᵢ ∂̄ψᵢ = ∂̄1 = 0` in chart `j`.
* **`conjForm`/`pairingDual_injective`** (Forster 19.5's positivity, localized): the conjugate
  `(0,1)`-form `θ̄` of a holomorphic `1`-form pairs against `θ` to `∑ᵢ ∫ ψᵢ |θᵢ|² ≥ 0`, zero only
...
* **`exists_dbar_of_forall_pairing_eq_zero`** — the integral-pairing Dolbeault bridge: a
  `(0,1)`-form pairing to zero against EVERY holomorphic `1`-form is `∂̄`-exact. Gated (exactly
  ...
  `H1Tail.equiv_of_surjective` + `h1T_zero_eq_genus`), whence the positivity injection is onto
...
`stokesA PU u θ i` is `∂̄ψᵢ · u · θ` read in chart `i`; `stokesB PU u θ i j` is its
...
private theorem stokes_step3 (i j : Fin PU.n) :
  ...
  · -- the transported identity on the overlap image in chart `j`
    ...
    -- T1: the ∂̄-chain rule on the transported partition function
...
/-- The conjugate `(0,1)`-form `θ̄` of a holomorphic `1`-form: chart coefficients
...
theorem finrank_H01_eq_genus
  ...
  have e1 : RS.LaurentTail.H1Tail (0 : RS.Divisor X) ≃ₗ[ℂ] RS.H01 X :=
    (RS.LaurentTail.H1Tail.equiv_of_surjective (0 : RS.Divisor X) hsurj).trans
      RS.dolbeaultEquiv
...
EVERY holomorphic `1`-form, it is `∂̄`-exact. Gated only on `serre-duality-tails`'s remaining
